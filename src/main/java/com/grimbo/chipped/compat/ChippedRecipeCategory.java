...
public class ChippedRecipeCategory implements DisplayCategory<ChippedRecipeCategory.FlattenedRecipe> {
	...
	public record FlattenedRecipe(Ingredient tag, ItemStack result, CategoryIdentifier<FlattenedRecipe> category) implements Display {
		@Override
		public List<EntryIngredient> getInputEntries() {
			return Collections.singletonList(EntryIngredient.of(
					Arrays.stream(this.tag.getItems()).map(item -> EntryStack.of(VanillaEntryTypes.ITEM, item)).collect(Collectors.toList())));
		}
